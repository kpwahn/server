...
router.post('/create-question', bodyParser.json(), function (req, res) {
  ...
  if( !request_check.has_correct_keys ){
    ...
    connection.getConnection(function (err, connection) {
        if (err) {
          ...
          connection.query(sql_statements.get_quiz, [req.body.quiz_id], function (err, rows) {
              if (err) {
                  res.status(status_codes.internal_server_error);
                ...
                if (rows.length !== 0) {
                  ...
                  connection.query(sql_statements.get_quesiton_by_question_and_quiz_id, [req.body.question, quiz_id], function (err, rows) {
                    if (err) {
                      res.status(status_codes.internal_server_error);
                      res.json({ message: constants.error_messages.db_query, err: err});
                    } else {
                      if (rows.length === 0 ){
                        connection.query(sql_statements.insert_question, [req.body.question, quiz_id], function (err, rows) {
                          ...
                          if (err) {
                            ...
                            res.json({ message: constants.success_messages.new_question, err: rows});
                          }
                        });
                      } else {
                  ...
                  res.status(status_codes.conflict);
                  res.json({ message: constants.error_messages.quiz_not_found, err: err});
                }
            }
        });
      }

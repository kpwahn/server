...
router.get('/books/:book_id/quizzes/:quiz_id/questions/:question_id', function (req, res) {
  connection.getConnection(function (err, connection) {
    if (err) {
      res.status(status_codes.internal_server_error);
      res.json({ message: constants.error_messages.db_connect, err: err});
    } else {
      connection.query(sql_statements.get_question, [req.params.question_id], function (err, rows) {
        connection.release();
        if (err) {
          res.status(status_codes.internal_server_error);
